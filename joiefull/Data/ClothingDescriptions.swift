//
//  ClothingDescriptions.swift
//  joiefull
//
//  Created by Julien Cotte on 19/12/2025.
//

/// Provides detailed descriptions for clothing items
/// Contains a dictionary mapping clothing IDs to their French descriptions
enum ClothingDescriptions {

    /// Dictionary mapping clothing item IDs to their detailed descriptions
    static let byId: [Int: String] = [
        0: """
        Sac à main orange en cuir lisse, aux lignes structurées et au rabat élégant,
        doté d’une chaîne métallique pour un porté épaule ou croisé.
        Raffiné et pratique au quotidien.
        """,
        1: """
        Jean noir à coupe ample, confectionné dans une matière structurée
        avec taille haute et lignes épurées.
        Moderne et facile à associer.
        """,
        2: """
        Bottes en cuir lisse à tige mi-haute, dotées d’une coupe structurée et d’un talon discret pour un porté confortable, offrant une silhouette élégante et intemporelle.
        nRobustes et faciles à associer.
        """,
        3: """
        Veste blazer à la coupe droite et structurée, confectionnée dans un tissu texturé avec finitions soignées et poches discrètes, apportant une allure élégante et contemporaine.
        Idéale pour un style habillé ou décontracté chic.
        """,
        4: """
        Pull vert forêt à motif torsadé élégant, tricot finement travaillé avec manches bouffantes et col montant.
        Doux et chaleureux.
        """,
        5: """
        Escarpins de soirée à talon fin, réalisés dans un cuir lisse au fini brillant avec brides délicates et détails élégants, offrant une silhouette raffinée et affirmée.
        Idéals pour sublimer une tenue habillée avec caractère.
        """,
        6: """
        Sac à dos d’inspiration outdoor, confectionné en toile résistante avec finitions en cuir et sangles ajustables, offrant un volume fonctionnel et un esprit authentique.
        Pensé pour accompagner les déplacements du quotidien comme les escapades prolongées.
        """,
        7: """
        Bomber à l’allure urbaine, réalisé dans une matière texturée avec doublure chaude et col contrasté, offrant une silhouette structurée et confortable.
        Idéal pour la mi-saison, entre élégance décontractée et protection légère.
        """,
        8: """
        Sweat à capuche au design épuré, confectionné dans un coton doux et épais avec finitions soignées et poche kangourou discrète.
        Une pièce confortable et moderne, idéale pour un style urbain affirmé au quotidien.
        """,
        9: """
        T-shirt au tombé naturel, confectionné en coton biologique doux et respirant avec finitions nettes et col rond épuré.
        Une pièce essentielle au style minimaliste, confortable et facile à associer au quotidien.
        """,
        10: """
        Pendentif délicat orné d’un médaillon bleu à l’aspect minéral, monté sur une chaîne fine aux lignes épurées.
        Une pièce discrète et lumineuse qui apporte une touche de couleur subtile, facile à porter au quotidien.
        """,
        11: """
        Pantalon noir aux lignes nettes et à la coupe ajustée, conçu dans une matière souple et agréable à porter.
        Une pièce sobre et polyvalente, idéale pour structurer une silhouette avec élégance et simplicité.
        """
    ]

    /// Default description used when a specific clothing item description is not available
    static let `default` = """
        Pièce essentielle au design soigné, pensée pour un usage quotidien.
        Coupe équilibrée, matière agréable et finitions discrètes
        pour un style simple et polyvalent.
        """
}
